...
public class UpdateSubject extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            request.getRequestDispatcher("/web_system/QA_28_EditSubject.jsp?classname=" + classname).forward(request, response);
            return; // 処理を中断
        }
        if (result) {
        	// 更新成功時は詳細画面へリダイレクト
        	response.sendRedirect(request.getContextPath() + "/DetailSubject?classname=" + classname);
        	return;
        } else {
